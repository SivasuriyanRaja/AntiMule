...
def health():
    ...
    if _ML_AVAILABLE:
        import os as _os
        models_dir = _os.path.join(_ROOT, "neon-guard-ui-main", "src", "lib", "models")
        model_loaded = _os.path.exists(_os.path.join(models_dir, "best_model.pkl"))
        if model_loaded:
            try:
                import joblib as _jl
                metrics_path = _os.path.join(
                    _ROOT, "neon-guard-ui-main", "src", "lib", "reports", "evaluation_metrics.json"
                )
                if _os.path.exists(metrics_path):
                    import json as _json
                    m = _json.load(open(metrics_path))
                    best = max(m, key=lambda x: x.get("avg_precision", 0))
...
async def predict_batch(
    file: UploadFile = File(..., description="CSV file — up to 500 MB"),
...
@app.get("/db/status")
async def db_status():
    """MongoDB + MySQL real-time connectivity check."""
    ...
    try:
        ...
        status["total_scored"] = stats.get("total_scored", 0)
        status["open_alerts"]  = stats.get("open_alerts", 0)
    except Exception:
        pass
    return status
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="0.0.0.0", port=8005, reload=True, limit_concurrency=100, timeout_keep_alive=300)
...
async def register(user: UserCreate):
    ...
    try:
        ...
        # Auto-generate token so frontend can auto-login
        payload = {
            'sub':   str(new_user.get('id', '')),
        ...
        return {
            'status': 'success',
            'token':  token,
            'user':   {'id': new_user.get('id'), 'email': new_user['email'], 'name': new_user.get('name', user.name)}
        }
