...
# ── Document builders ─────────────────────────────────────────────────────────
def _pred_doc(account_data: dict, result: dict, source: str = "api", user_id: str = None) -> dict:
    return {
...
async def async_save_prediction(account_data: dict, result: dict,
                                source: str = "api", user_id: str = None) -> str:
    db  = get_async_db()
...
async def async_create_user(email: str, password_hash: str, name: str = "") -> dict:
    db = get_async_db()
    existing = await db.users.find_one({'email': email})
    if existing:
        raise ValueError(f'An account with email {email} already exists.')
    doc = {'email': email, 'password_hash': password_hash, 'name': name,
